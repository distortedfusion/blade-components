...
<div data-slot="list-group-item" {{ $componentAttributes->class([
    ...
    @if(! is_null($href) || $button)
        {{-- hover-indicator --}}
        <div class="absolute inset-x-1 inset-y-1 z-0 scale-95 bg-[color-mix(in_oklab,var(--secondary),var(--foreground)_5%)] rounded opacity-0 transition group-hover:scale-100 group-hover:opacity-100"></div>
    @endif
